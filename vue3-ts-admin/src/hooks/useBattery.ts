...
interface BatteryModal {
  battery: Battery;
}

export const useBattery = () => {
  const state = reactive<BatteryModal>({
    battery: {
      charging: false,
      chargingTime: 0,
      dischargingTime: 0,
  ...
  onMounted(async () => {
    ...
    // 电池充电状态更新时被调用
    BatteryManager.onchargingchange = ({ target }: Battery) => {
      updateBattery(target);
    };
    // 电池充电时间更新时被调用
    BatteryManager.onchargingtimechange = ({ target }: Battery) => {
      updateBattery(target);
    };
    // 电池断开充电时间更新时被调用
    BatteryManager.ondischargingtimechange = ({ target }: Battery) => {
      updateBattery(target);
    };
    // 电池电量更新时被调用
    BatteryManager.onlevelchange = ({ target }: Battery) => {
      updateBattery(target);
    };
