...
export const exportExcel = async ({
  ...
  if (nav.msSaveOrOpenBlob) {
    ...
    const link = window.document.createElement("a"); // a标签下载
 ...
 * 导出多表头excel
 */
export const exportMultiHeaderExcel = ({ column, data, filename }: any) => {
  // 创建excel
  const workbook = new ExcelJS.Workbook();
  // 创建工作表
  ...
  column.forEach((item: any) => {
    if (item._children) {
      item._children.forEach((itemChild: any) => {
        ...
        const maxArr = [
          autoWidthAction(itemChild.label ? itemChild.label : ""),
        ];
        data.forEach((ite: any) => {
