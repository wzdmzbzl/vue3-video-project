...
import { useUserStore } from "@/store/modules/user"; // 这个功能还没实现

// 功能失败的错误原因
export const authReeMessage: any = {
  10031: "登录失效，需要重新登录",
  10032: "您太久没登录，请重新登录~",
...
request.interceptors.response.use(
  (response: any) => {
    ...
    if (code === 200) {
      ...
      let errMessage = authReeMessage[code];
      if (errMessage) {
  ...
  (error: any) => {
    ...
    if (error.response) {
      // error.response有值，说明服务器有响应，响应结果是错误的，要根据响应状态码error.response.st
